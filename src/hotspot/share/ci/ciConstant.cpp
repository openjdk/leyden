...
// ------------------------------------------------------------------
// ciConstant::print
void ciConstant::print_on(outputStream* st) {
  st->print("<ciConstant type=%s value=",
             basictype_to_str(basic_type()));
  switch (basic_type()) {
  case T_BOOLEAN:
    st->print("%s", bool_to_str(_value._int != 0));
    break;
  case T_CHAR:
  case T_BYTE:
  case T_SHORT:
  case T_INT:
    st->print("%d", _value._int);
    break;
  case T_LONG:
    st->print(INT64_FORMAT, (int64_t)(_value._long));
    break;
  case T_FLOAT:
    st->print("%f", _value._float);
    break;
  case T_DOUBLE:
    st->print("%lf", _value._double);
    break;
  default:
    if (is_reference_type(basic_type())) {
      _value._object->print_on(st);
    } else {
      st->print("ILLEGAL");
    }
    break;
