...
public:
  // Can this VM write a heap region into the CDS archive? Currently only {G1|Parallel|Serial|Shenandoah}+compressed_cp
  static bool can_write() {
    CDS_JAVA_HEAP_ONLY(
      if (_disable_writing) {
        return false;
      }
      return (UseG1GC || UseParallelGC || UseSerialGC || UseShenandoahGC) && UseCompressedClassPointers;
    )
