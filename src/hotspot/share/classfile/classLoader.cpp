...
char* ClassLoader::lookup_vm_options() {
  ...
  bool hermetic = (Arguments::hermetic_jdk_image_path() != nullptr);
  ...
  if (JImage_file == nullptr) {
    if (Arguments::has_jimage()) {
      ...
      vm_exit_during_initialization(err_msg("Unable to load %s",
        hermetic ? Arguments::hermetic_jdk_image_path() : modules_path));
    }
    return nullptr;
  }
