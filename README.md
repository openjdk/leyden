...
The purpose of the Leyden repository is for prototyping improvements of
...
The Leyden repo is closely tracking the JDK main-line development. We are typically only a few weeks behind
the [main-line JDK repo](https://github.com/openjdk/jdk). We have implemented the following improvements
over the JDK main-line:

- <b>[Unified Cache Data Storage (JDK-8320264)](https://openjdk.org/jeps/8320264)</b> : This is an enhancement
  over [CDS](https://docs.oracle.com/en/java/javase/22/vm/class-data-sharing.html):
  - In addition to class metadata and heap objects, now CDS can also be used to store profiling data, as well as
    compiled Java methods.
  - We have simplified the creation of the CDS archive to make it easy to use CDS in your Java applications.
  - This feature is accessed with the new VM flag `-XX:CacheDataStore`.
- <b>[Loaded Classes in CDS Archives (JDK-8315737)](https://openjdk.org/jeps/8315737)</b> : This gives
  ...
  - This feature is accessed with the new VM flag `-XX:+PreloadSharedClasses`. (Note, this flag will be renamed when JDK-8315737
...
- <b>Ahead-of-time generation of reflection data</b>: Reflection data (such as instances of
  ...
  - This feature is enabled by the new VM flags `-XX:+ArchiveReflectionData`.
- <b>Class Loader Lookup Cache</b>: Sometimes application frames may perform many lookup of classes by name (with `Class.forName()`,
  etc.). This optimization allows such lookup to be done quickly without scanning the classpath.
  - This feature is enabled by the new VM flags `-XX:+ArchiveLoaderLookupCache`.

## 2. Building the Leyden Repo

The Leyden Repo can be built in the same way as the main-line JDK repo.
...
The easiest way to try out the Leyden features is to build a JVM from the Leyden repo, and use it with your application with the `-XX:CacheDataStore` flag.
...
setting up the classes used by `JavaCompiler`, so it will will benefit from the Leyden features.
...
(Remember to use the `java` program that you built from the Leyden repo)
...
<b>Note that you must delete any existing `JavacBenchApp.cds*` files
before this step</b>:
...
- JavacBenchApp.cds: this file contains the classes, heap objects and profiling data
- JavacBenchApp.cds.code: this file contains the AOT-compiled methods (this file will be
  merged into JavacBenchApp.cds in a future release).

Now, we can make a <b>production run</b> of the program with the cache files. It finishes in 423 ms, or more than 50% faster than
before.
...
When you create the `JavacBenchApp.cds` file with the  `-XX:CacheDataStore` flag, all of the other options described
...
starts more slowely than with loading AOT code.
...
You can also disable AOT compilation the training run:
...
```
# training run (uses G1 bu default)
$ rm -fv JavacBenchApp.cds*
$ java -XX:CacheDataStore=JavacBenchApp.cds -cp JavacBenchApp.jar JavacBenchApp 50
...
[0.001s][error][cds] CDS archive has preloaded classes. It cannot be used because GC used during dump time (G1) is not the same as runtime (Serial)
...
Error occurred during initialization of VM
Cannot create the CacheDataStore: UseCompressedClassPointers must be enabled, and collector must be G1, Parallel, or Serial
```
...
continue to run without using CDS. This ensures that the application's functionality by falling back
...
### Spring PetClinic Demo (2.71x improvement)
