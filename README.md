...
```
$ java -Xlog:scc=error -XX:CacheDataStore=JavacBenchApp.cds -cp JavacBenchApp.jar JavacBenchApp 50
Generated source code for 51 classes and compiled them in 423 ms
```

(Temporary: the flag `-Xlog:scc=error` is necessary to avoid some verbose logging output).
