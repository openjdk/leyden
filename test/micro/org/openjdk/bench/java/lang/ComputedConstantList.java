...
public class ComputedConstantList {
    ...
    public List<DoubleChecked<Integer>> doubleChecked;

    @Setup(Level.Iteration)
    public void setupIteration() {
        listOfConstants = ComputedConstant.of(SIZE, MAPPER);;
        listOfConstantsNull = ComputedConstant.of(SIZE, NULL_MAPPER);
        doubleChecked = IntStream.range(0, SIZE)
    ...
    public void constant(Blackhole bh) {
        bh.consume(listOfConstants.get(POS).get());
    }

    @Benchmark
    public void constantNull(Blackhole bh) {
        bh.consume(listOfConstantsNull.get(POS).get());
    }

    @Benchmark
    public void doubleChecked(Blackhole bh) {
        bh.consume(doubleChecked.get(POS).get());
    }
